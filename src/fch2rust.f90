...
! updated by wsr at 20211029: turn fch2qchem into fch2rust, follow libcint order
...
! character(len=5) :: str2
 ...
 case(1)
 case default
  write(iout,'(/,A)') ' ERROR in subroutine fch2rust: wrong command line arguments!'
  write(iout,'(A)')   ' Example 1 (R(O)HF, UHF): fch2rust a.fch'
  stop
 end select
...
i = INDEX(fchname,'.fch',back=.true.)
 if(i == 0) then
  write(iout,'(A)') "ERROR in subroutine fch2rust: input filename does not&
                     & contain '.fch' suffix!"
...
! base_qc = TRIM(basename)//'_qc'
 ...
 if(.not. nobasistransform_in_fch(fchname)) then
  ...
  write(iout,'(A)') "Warning in subroutine fch2rust: keyword 'nobasistransform'&
...
!  write(iout,'(A)') 'ERROR in subroutine fch2rust: inconsistent basis sets in&
!                   & .fch file and in libcint script, ncoeff/=nbf*nif!'
...
! rewind(fchid)
 ...
 if(i /= 0) then
  write(iout,'(A)') "ERROR in subroutine fch2rust: missing the 'Shell types'&
                   & section in .fch file!"
  write(iout,'(A)') TRIM(fchname)
  close(fchid)
  stop
 ...
 if(i /= 0) then
  write(iout,'(A)') "ERROR in subroutine fch2rust: missing the 'Shell to atom map'&
                   & section in .fch file!"
  write(iout,'(A)') TRIM(fchname)
  close(fchid)
  stop
...
! then we adjust the basis functions in each MO according to the type of basis functions
 ...
 do i = 1,n5dmark,1
  call fch2rust_permute_5d(nif,alpha_coeff(d_mark(i):d_mark(i)+4,:))
 end do
 do i = 1,n6dmark,1
  call fch2rust_permute_6d(nif,alpha_coeff(d_mark(i):d_mark(i)+5,:))
 end do
 do i = 1,n7fmark,1
  call fch2rust_permute_7f(nif,alpha_coeff(f_mark(i):f_mark(i)+6,:))
 end do
 do i = 1,n10fmark,1
  call fch2rust_permute_10f(nif,alpha_coeff(f_mark(i):f_mark(i)+9,:))
 end do
 do i = 1,n9gmark,1
  call fch2rust_permute_9g(nif,alpha_coeff(g_mark(i):g_mark(i)+8,:))
 end do
 do i = 1,n15gmark,1
  call fch2rust_permute_15g(nif,alpha_coeff(g_mark(i):g_mark(i)+14,:))
 end do
 do i = 1,n11hmark,1
  call fch2rust_permute_11h(nif,alpha_coeff(h_mark(i):h_mark(i)+10,:))
 end do
 do i = 1,n21hmark,1
  call fch2rust_permute_21h(nif,alpha_coeff(h_mark(i):h_mark(i)+20,:))
 end do
! adjustment finished
if (uhf) then
 ! adjust the order of d, f, etc. functions
 do i = 1,n5dmark,1
  call fch2rust_permute_5d(nif,beta_coeff(d_mark(i):d_mark(i)+4,:))
 end do
 do i = 1,n6dmark,1
  call fch2rust_permute_6d(nif,beta_coeff(d_mark(i):d_mark(i)+5,:))
 end do
 do i = 1,n7fmark,1
  call fch2rust_permute_7f(nif,beta_coeff(f_mark(i):f_mark(i)+6,:))
 end do
 do i = 1,n10fmark,1
  call fch2rust_permute_10f(nif,beta_coeff(f_mark(i):f_mark(i)+9,:))
 end do
 do i = 1,n9gmark,1
  call fch2rust_permute_9g(nif,beta_coeff(g_mark(i):g_mark(i)+8,:))
 end do
 do i = 1,n15gmark,1
  call fch2rust_permute_15g(nif,beta_coeff(g_mark(i):g_mark(i)+14,:))
 end do
 do i = 1,n11hmark,1
  call fch2rust_permute_11h(nif,beta_coeff(h_mark(i):h_mark(i)+10,:))
 end do
 do i = 1,n21hmark,1
  call fch2rust_permute_21h(nif,beta_coeff(h_mark(i):h_mark(i)+20,:))
 end do
! adjustment finished
!else
...
!!! if (.not. sph) then
!!!  write(iout,'(A)') 'Warning in subroutine fch2rust: Cartesian basis &
!!!                   &convertion not implemented. $basis will be left blank.'
!!!  return
!!! endif
!!! isys = system('fch2mkl '//TRIM(fchname))
!!! if(isys /= 0) then
!!!  write(iout,'(A)') 'ERROR in subroutine fch2rust: call fch2mkl failed.'
...
subroutine fch2rust_permute_5d(nif,coeff)
 implicit none
 integer :: i
...
! To: the order of spherical d functions in libcint
! 1    2    3    4    5
! d0 , d+1, d-1, d+2, d-2
! d-2, d-1, d0 , d+1, d+2
 ...
 deallocate(coeff2)
 return
end subroutine fch2rust_permute_5d

subroutine fch2rust_permute_6d(nif,coeff)
...
! To: the order of Cartesian d functions in libcint
! 1  2  3  4  5  6
! XX,YY,ZZ,XY,XZ,YZ
! XX,XY,YY,XZ,YZ,ZZ
 ...
 deallocate(coeff2)
 return
end subroutine fch2rust_permute_6d

subroutine fch2rust_permute_7f(nif,coeff)
 implicit none
 integer :: i
...
! To: the order of spherical f functions in libcint
! 1    2    3    4    5    6    7
...
! f-3, f-2, f-1, f0 , f+1, f+2, f+3
 ...
 deallocate(coeff2)
 return
end subroutine fch2rust_permute_7f

subroutine fch2rust_permute_10f(nif,coeff)
...
! To: the order of Cartesian f functions in libcint
...
! XXX,XXY,XYY,YYY,XXZ,XYZ,YYZ,XZZ,YZZ,ZZZ
 ...
 deallocate(coeff2)
 return
end subroutine fch2rust_permute_10f

subroutine fch2rust_permute_9g(nif,coeff)
 implicit none
 integer :: i
...
! To: the order of spherical g functions in libcint
! 1    2    3    4    5    6    7    8    9
...
! g-4, g-3, g-2, g-1, g0 , g+1, g+2, g+3, g+4
 ...
 deallocate(coeff2)
 return
end subroutine fch2rust_permute_9g

subroutine fch2rust_permute_15g(nif,coeff)
...
! To: the order of Cartesian g functions in libcint
...
! forall(i = 1:15) coeff(i,:) = coeff2(order(i),:) !/Sdiag_g(i)
 ...
 deallocate(coeff2)
 return
end subroutine fch2rust_permute_15g

subroutine fch2rust_permute_11h(nif,coeff)
 implicit none
 integer :: i
...
! To: the order of spherical h functions in libcint
...
! h-5, h-4, h-3, h-2, h-1, h0 , h+1, h+2, h+3, h+4, h+5
 ...
 deallocate(coeff2)
 return
end subroutine fch2rust_permute_11h

subroutine fch2rust_permute_21h(nif,coeff)
...
! To: the order of Cartesian h functions in libcint
...
! forall(i = 1:21) coeff(i,:) = coeff2(order(i),:) !/Sdiag_h(i)
 ...
 deallocate(coeff2)
 return
end subroutine fch2rust_permute_21h
